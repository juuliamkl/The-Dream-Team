...
export const addScoreForStudents = (projectId: Project["id"], token: AuthToken, setStudents: Function) => {
    return (students: StudentWithLocation[]): void => {
        ...
        const update = (projectScores: ProjectScore) => {
            ...
            students.forEach(wrappedStudent => {
                const {Score, motivation} = projectScores.scores.filter(score => score.studentId === wrappedStudent.student.id)[0];
                out.push(
                    {
                        ...wrappedStudent,
                        student:
